# MsgRelay — 隱私政策（Privacy Policy）

生效日期：2026 年

**一句話總結：MsgRelay 不收集任何數據。所有 WhatsApp 數據 100% 存儲在您自己的設備上。**

---

## 1. 我們收集什麼

**我們（MsgRelay 開發者）不收集任何個人信息。** 具體而言：
...
MsgRelay 的功能需要與您自己的第三方帳戶互動。**這些連接由您發起、以您的身份進行**：
...
- **LLM 增強（可選功能，默認關閉）**：僅當您主動配置 API key 後，消息文本才會發送給**您指定的 API 提供方**（OpenAI / DeepSeek / Ollama / 其他兼容服務）用於語義提取。
  - 使用本地模型（如 Ollama）時，數據不出您的設備
  - 使用雲端 API 時，消息文本會發送給該提供方，受其隱私政策約束
  - **不配置 LLM = 零外部數據流**，所有提取均在本地完成

## 4. 數據控制者與處理者

- **您**（用戶）是您個人數據的**控制者（Data Controller）**——您決定處理什麼、怎麼處理
- **MsgRelay 開發者**不是數據處理者：我們不訪問、不存儲、不處理您的任何數據
